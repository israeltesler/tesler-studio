...
const NAV_ITEMS: readonly NavItem[] = [
  ...
  ...(PROJECTS_PAGE_ENABLED
    ? [{ label: "פרויקטים", href: "/projects" } satisfies NavItem]
    : []),
...
export function Nav(): ReactNode {
  ...
  useLayoutEffect(() => {
    ...
    if (!list || !activeEl) {
      const frame = requestAnimationFrame(() => setPillRect(null));
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1 rounded-full bg-background p-1.5 shadow-sm border border-foreground/8">
        ...
        <span
          aria-hidden="true"
          className="mx-0.5 h-4 w-px shrink-0 bg-foreground/10"
        />
        <Link
          href="/"
          aria-label="tesler studio — דף הבית"
          className="focus-ring inline-flex cursor-pointer items-center rounded-full bg-neutral-950 px-2.5 py-1.5"
        >
          <StudioWordmark size="header" tone="light" />
        </Link>
      </div>
    </nav>
  );
